package com.example.spring.intbatch.dbtool;

import org.springframework.jdbc.core.JdbcTemplate;

import com.example.spring.intbatch.entity.BankTransaction;
...
public class SourceManagementUtils {
    ...
    public static void addBalanceColumn(DataSource dataSource) {
        ...
        jdbcTemplate.update("alter table bankingschema.bank_transaction_yearly add column if not exists balance numeric(10,2)");
    }
    ...
    public static void addAdjustedColumn(DataSource dataSource) {
        ...
        jdbcTemplate.update("alter table bankingschema.bank_transaction_yearly add column if not exists adjusted boolean default false");
    }
    ...
    public static void initializeEmptyDatabase(JdbcTemplate jdbcTemplate) {
        // Drop table if exist
        jdbcTemplate.update("drop table if exists bankingschema.bank_transaction_yearly");
        // Create the table
        jdbcTemplate.update("create table bankingschema.bank_transaction_yearly (" +
    ...
    public static void insertBankTransaction(BankTransaction transaction, JdbcTemplate jdbcTemplate) {
        jdbcTemplate.update("insert into bankingschema.bank_transaction_yearly (month, day, hour, minute, amount, merchant) " +
                        "values (?, ?, ?, ?, ?, ?)",
